...
class Apptha_Marketplace_Block_Product_New extends Mage_Core_Block_Template {
    ...
    public function show_categories_tree($customerName) {
           // Mage::log($customerName, null, 'mylogfile.log');
        ...
        foreach ( $customerName as $key => $catname ) {
            ...
            if (strstr ( $key, 'sub' )) {
                ...
                $array .= '<li class="level-top  parent" id="' . $key . '"><a href="javascript:void(0);"><span class="end-plus" id="' . $key . '"></span></a><span class="last-collapse"><input id="cat' . $key . '" type="checkbox" name="category_ids[]" value="' . $key . '"><label for="cat' . $key . '">' . $catagory->getName() . ' <span>(' . $count . ')</span>' . '</label></span>';
            } else { 
                $array .= '<li class="level-top  parent"><a href="javascript:void(0);"><span class="empty_space"></span></a><input id="cat' . $key . '" type="checkbox" name="category_ids[]" value="' . $key . '"><label for="cat' . $key . '">' . $catagory->getName() . ' <span>(' . $count . ')</span>' . '</label>';
            }
