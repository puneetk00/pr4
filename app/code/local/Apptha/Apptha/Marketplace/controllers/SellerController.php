...
class Apptha_Marketplace_SellerController extends Mage_Core_Controller_Front_Action {
    ...
    function saveprofileAction() {
		...
		*/
         ...
         */
        $documents = $imagesPathBanner = $imagesPathLogo = '';
        /**
		...
		$pan = $data['pan'];  
		$tan = $data['tan'];
		$cancel_cheque = $vat_certificate = $address_proof = $owner_identity_proof = $signature = "";
        ...
        $document = $filesDataArray ['document'] ['name'];
		$cancel_cheque = $filesDataArray ['cancel_cheque'] ['name'];
        $vat_certificate = $filesDataArray ['vat_certificate'] ['name'];
        $address_proof = $filesDataArray ['address_proof'] ['name'];
        $owner_identity_proof = $filesDataArray ['owner_identity_proof'] ['name'];
        $signature = $filesDataArray ['signature'] ['name'];
        
        /**
         * Get Media base Directory
         */
        $basedir = Mage::getBaseDir ( 'media' );
        $docpath = Mage::getBaseDir ( 'media' ) . DS . 'sellerimage' . DS . $sellerId;
        /**
        ...
        $documents = Mage::getModel('marketplace/seller')->docUpload ( $filesDataArray, $docpath, 'document' );
		$cancel_cheque = Mage::getModel('marketplace/seller')->docUpload( $filesDataArray, $docpath, 'cancel_cheque' );
        $vat_certificate = Mage::getModel('marketplace/seller')->docUpload( $filesDataArray, $docpath, 'vat_certificate' );
        $address_proof = Mage::getModel('marketplace/seller')->docUpload( $filesDataArray, $docpath, 'address_proof' );
        $owner_identity_proof = Mage::getModel('marketplace/seller')->docUpload( $filesDataArray, $docpath, 'owner_identity_proof' );
        $signature = Mage::getModel('marketplace/seller')->docUpload( $filesDataArray, $docpath, 'signature' );
        $collection = Mage::getModel('marketplace/sellerprofile')->load($sellerId,'seller_id');
        $getId = $collection->getId ();
		//$query = "insert into marketplace_sellerdoc (id,seller_id,doc) values(null,'{$sellerId}','{$documents}')";
		$selectquery = "select * from marketplace_sellerdoc where seller_id = '{$sellerId}'";
		$results = $readConnection->fetchAll($selectquery);

		$cancel_chequesv = $results[0]['cancel_cheque'];
		$vat_certificatesv = $results[0]['vat_certificate'];
		$address_proofsv = $results[0]['address_proof'];
		$owner_identity_proofsv = $results[0]['owner_identity_proof'];
		$signaturesv = $results[0]['signature'];

		//print_r($cancel_cheque); die;
		$cancel_cheque = $cancel_cheque!=''? "`cancel_cheque` = '{$cancel_cheque}',":"`cancel_cheque` = '{$cancel_chequesv}',";
		$vat_certificate = $vat_certificate!=''? "`vat_certificate` = '{$vat_certificate}',":"`vat_certificate` = '{$vat_certificatesv}',";
		$address_proof = $address_proof!=''? "`address_proof` = '{$address_proof}',":"`address_proof` = '{$address_proofsv}',";
		$owner_identity_proof = $owner_identity_proof!=''? "`owner_identity_proof` = '{$owner_identity_proof}',":"`owner_identity_proof` = '{$owner_identity_proofsv}',";
		$signature = $signature!=''? "`signature` = '{$signature}',":"`signature` = '{$signaturesv}',";
		
		
		$query = "replace into marketplace_sellerdoc set `seller_id` = '{$sellerId}',`doc` = '{$documents}',`gstin` = '{$gstin}',`pan` = '{$pan}', `tan` = '{$tan}',{$cancel_cheque}{$vat_certificate}{$address_proof}{$owner_identity_proof}{$signature}";
		//print_r($query); die;
		$query = rtrim($query,',');
